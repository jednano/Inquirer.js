...
export class Expression<T extends ExpressionType> {
  ...
  public toString() {
    switch (typeof this.value) {
      ...
      default:
        throw new TypeError('Expression must be a string, number or function.');
    }
  }
}
...
> extends Array<[Expression<L>, Operator, Expression<R>]> {
  ...
  private end(operator: Operator, rightExpression: R) {
    this.push([this.leftExpression, operator, new Expression(rightExpression)]);
...
> {
  ...
  private chain<T extends ExpressionType>(conjunction: Conjunction, expression: T) {
...
app.extend({
  search(query) {
    return query.and('color').eq('red').or('price').lte(100)
  }
});
